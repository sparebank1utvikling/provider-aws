...
type ipPermissionMap struct {
	...
	groups        map[string]awsec2.UserIdGroupPair
}
...
func (i *ipPermissionMap) merge(m awsec2.IpPermission) { // nolint:gocyclo
	...
	for _, r := range m.UserIdGroupPairs {
		...
		key := aws.StringValue(r.GroupId)
		if key == "" {
			key = aws.StringValue(r.GroupName)
		}
		i.groups[key] = r
	}
}
...
func (i ipPermissionMap) diffUserIDGroupPair(other ipPermissionMap) []awsec2.UserIdGroupPair {
	...
	for key, r := range i.groups {
		...
		if !ok || aws.StringValue(r.Description) != aws.StringValue(r2.Description) {
...
func convertToMaps(rules []awsec2.IpPermission) map[ruleKey]*ipPermissionMap {
	...
	for _, rule := range rules {
		...
		if !ok {
			...
			normalized.groups = make(map[string]awsec2.UserIdGroupPair)
			ret[k] = normalized
		}
